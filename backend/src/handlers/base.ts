...
export default abstract class BaseHttpHandler<T extends object, K extends Key = number> {
    ...
    public async getAll(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            if (!this.validadorRequest.isQuery(req.query)) throw new Error("Formato de query no válido.");
            if (Object.keys(req.params || {}).length > 0 && !this.validadorRequest.isParam(req.params)) {
    ...
    public async update(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            if (!this.validadorRequest.isParam(req.params)) throw new Error("Formato de params no válido.");
    ...
    // DELETE: soporta { Ids:number[] } o { Keys:K[] }
    public async deleteMany(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            let ids: K[] | null = null;
            if (this.validadorRequest.isBodyIds?.(req.body)) ids = req.body.Ids as K[];
            else if (this.validadorRequest.isBodyKeys?.(req.body)) ids = req.body.Keys as K[];

            if (!ids || ids.length === 0) throw new Error("Formato del body no válido (Ids[] o Keys[]).");
