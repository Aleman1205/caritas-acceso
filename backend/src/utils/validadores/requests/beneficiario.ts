...
/**
 * Validador de Beneficiario
 * Valida el formato de datos para crear, modificar o consultar beneficiarios.
 */
export default class BeneficiarioValidador extends BaseValidadorRequest<Beneficiario> {
	...
	public override isBodyIds(obj: Request["body"]): boolean {
		return (
			...
			obj.Telefonos.every(
				(t: unknown) => typeof t === "string" && isValidString(t, 25)
			)
		);
	}

	// Convierte los filtros de query a un objeto Beneficiario (para DB)
	public override getFiltrosQuery(
		obj: Request["query"]
