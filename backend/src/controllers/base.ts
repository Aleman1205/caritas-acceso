...
export default abstract class BaseController<T, K extends Key = number> {
	...
	// Nuevo método: getById
	public async getById(id: K): Promise<T | null> {
		if (typeof (this.dbService as any).getById === "function") {
			return await (this.dbService as any).getById(id);
		}
		throw new Error("getById() no está implementado en el DB service.");
	}
}
