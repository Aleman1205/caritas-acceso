...
export default function ServiciosPage() {
  ...
  const normalizeServicio = (s: any): Servicio => ({
    ...
    Estatus: s.estatus === true || s.estatus === 'true' || s.estatus === 1,
  });
  ...
  async function handleBuscar() {
    ...
    try {
      const url = q.trim()
        ? `${BACKEND_URL}/web/servicio.routes/${encodeURIComponent(q)}`
        : `${BACKEND_URL}/web/servicio.routes`;
      const res = await fetch(url);
      if (!res.ok) throw new Error(`Backend respondió ${res.status}`);
      const json = await res.json();

      const rawRows: any[] = Array.isArray(json.data) ? json.data : [];
      const normalized = rawRows.map(normalizeServicio);
      setRows(normalized);

      if (normalized.length === 0) setErrorMsg('No se encontraron resultados.');
    } catch (err: any) {
      setErrorMsg(err?.message || 'Error al buscar servicios');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/web/servicio.routes`, {
        method: 'POST',
  ...
  async function handleUpdate() {
    ...
    try {
      const body = { nombre: nombre.trim(), descripcion: descripcion.trim() || undefined, estatus: activo };
      const res = await fetch(`${BACKEND_URL}/web/servicio/${editId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      });
  ...
  async function handleDeleteOne(id: number) {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/web/servicio/${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error(`Backend respondió ${res.status}`);
      await res.json();
